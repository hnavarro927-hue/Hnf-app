/**
 * Motor Jarvis Operativo v1 — clasificación, coincidencias y distribución (lógica local, sin IA externa).
 * Espejo funcional de frontend/domain/jarvis-intake-engine.js
 */
...
/**
 * @param {object} ctx
 * @param {{ nombre?: string, telefono?: string, email?: string, whatsapp?: string }[]} ctx.clientes
 * @param {{ cliente?: string, telefonoContacto?: string }[]} ctx.otsMuestra
 * @param {{ patente?: string, nombre?: string }[]} [ctx.vehicles]
 * @param {string[]} [ctx.technicians]
 */
...
/**
 * @param {object} input
 * @param {object} input.matchContext - ver matchEntities
 * @param {string} [input.actorIngreso]
 */
...
/** Aplana brief para persistir en jarvis_operative_events.json */
...
/** Objeto compacto para guardar en OT (JSON-serializable) */
