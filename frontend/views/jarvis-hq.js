...
function buildMaestroIntakeOperativoStrip(io) {
  ...
  const lines = [
    ...
    `Costo ejecutado (mes, OT): ${fmtCLP(io.kpi_fin_costo_ejecutado_mes_clp)}`,
    `Ingreso estimado mensual (OT mant., período): ${fmtCLP(io.kpi_fin_ingreso_estimado_mensual_clp)}`,
    `Ingreso facturado real (mes): ${fmtCLP(io.kpi_fin_ingreso_facturado_mes_clp)}`,
    `Utilidad estimada (mant. mensual): ${fmtCLP(io.kpi_fin_utilidad_estimada_mensual_clp)}`,
    `Utilidad facturada (mes): ${fmtCLP(io.kpi_fin_utilidad_facturada_mes_clp)}`,
